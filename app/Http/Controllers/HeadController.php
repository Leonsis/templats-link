...
class HeadController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($temaAtivo === 'main-Thema') {
            if ($request->ajax() || $request->expectsJson()) {
                return response()->json(['error' => 'As configurações de Head são aplicadas apenas aos temas personalizados. O tema principal (main-Thema) usa dados fixos.'], 400);
            }
    ...
    public function update(Request $request)
    {
        ...
        if ($temaAtivo === 'main-Thema') {
            if ($request->ajax() || $request->expectsJson()) {
                return response()->json(['error' => 'As configurações de Head são aplicadas apenas aos temas personalizados.'], 400);
            }
        ...
        $request->validate([
            ...
            'meta_description.max' => 'A descrição deve ter no máximo 300 caracteres.',
        ...
        try {
            ...
            if ($request->ajax() || $request->expectsJson()) {
                return response()->json(['success' => true, 'message' => "Configurações da página '{$paginaNome}' salvas com sucesso!"]);
            }
