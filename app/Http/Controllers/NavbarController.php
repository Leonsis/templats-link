...
class NavbarController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($temaAtivo === 'main-Thema') {
            if ($request->ajax() || $request->expectsJson()) {
                return response()->json(['error' => 'As configurações de NavBar/Footer são aplicadas apenas aos temas personalizados. O tema principal (main-Thema) usa dados fixos.'], 400);
            }
    ...
    public function update(Request $request)
    {
        ...
        if ($temaAtivo === 'main-Thema') {
            if ($request->ajax() || $request->expectsJson()) {
                return response()->json(['error' => 'As configurações de NavBar/Footer são aplicadas apenas aos temas personalizados.'], 400);
            }
        ...
        $request->validate([
            ...
            'github.url' => 'Digite uma URL válida para o GitHub.',
        ]);
        ...
        try {
            ...
            if ($request->ajax() || $request->expectsJson()) {
                if ($request->has('save_logo')) {
                    return response()->json(['success' => true, 'message' => 'Logo Navbar salvo com sucesso!']);
                } elseif ($request->has('save_logo_footer')) {
                    return response()->json(['success' => true, 'message' => 'Logo Footer salvo com sucesso!']);
                } else {
                    return response()->json(['success' => true, 'message' => 'Configurações da Navbar/Footer salvas com sucesso!']);
                }
            }
