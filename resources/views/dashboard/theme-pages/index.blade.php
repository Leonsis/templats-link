                        ...
                        <h5 class="card-title mb-0">
                         ...
                         @if($userCanAccessThemes)
                             ...
                             <button type="button" 
                                     ...
                                     onclick="generateSitemap('{{ $temaAtivo }}')"
                             ...
                             <button type="button" 
                                     ...
                                     onclick="generateLlms('{{ $temaAtivo }}')"
                                            ...
                                            <div class="d-flex align-items-start justify-content-between mb-2">                                                                                                    
                                                <div class="flex-grow-1 min-w-0">                                                        
                                                    <small class="text-muted d-block text-truncate" style="font-size: 0.75rem;">
                                                        <i class="fas fa-code me-1"></i>{{ $pagina }}.blade.php
                                                    </small>
                                                </div>                                                
                                            </div>
                                            ...
                                            <div class="d-flex align-items-center justify-content-between">
                                                @if($temConfiguracao)
                                                    <span class="badge bg-success" style="font-size: 0.7rem;">
                                                        ...
                                                        {{ $configCount }}/3 Configurado{{ $configCount > 1 ? 's' : '' }}
                                                    </span>
                                                @else
                                            ...
                                            @if(isset($rotasPaginas[$pagina]))
                                                ...
                                                <div class="mb-3 p-2 bg-light rounded" style="font-size: 0.8rem;">
                                                    ...
                                                    <code class="text-info d-block text-truncate" style="font-size: 0.75rem;">
                                                        @if($infoRota['tipo'] === 'nomeada' && $infoRota['rota'])
                                                            {{ str_replace(['route(', ')', "'"], '', $infoRota['rota']) }}
                                                        @elseif($infoRota['url'])
                                                            {{ $infoRota['url'] }}
                                                        @endif
                                                    </code>
                                                ...
                                                @if($userCanAccessThemes)
                                                    <div class="btn-group w-100" role="group">
                                                        <button type="button" 
                                                                ...
                                                                onclick="openRenamePageModal('{{ $pagina }}')"
                                                        ...
                                                        @if(!in_array($pagina, ['home', 'sobre', 'contato', 'blog', 'blogs', 'politicas-de-privacidade']))
                                                            <button type="button" 
            ...
            <form id="renamePageForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" 
                               class="form-control @error('novo_nome_pagina') is-invalid @enderror" 
                               id="novoNomePagina" 
                               name="novo_nome_pagina" 
                               placeholder="Digite o novo nome da página"
                               required>
                        @error('novo_nome_pagina')
...
function openRenamePageModal(pageName) {
    ...
    document.getElementById('renamePageForm').action = '{{ route("dashboard.theme-pages.rename", ":page") }}'.replace(':page', pageName);
...
function generateSitemap(nomeTema) {
    ...
    if (confirm('Deseja gerar o sitemap.xml para o tema ativo do sistema?\n\nIsso irá analisar todas as páginas do tema e criar um arquivo sitemap.xml na raiz do site.')) {
        ...
        const actionUrl = '{{ route("dashboard.temas.generate-sitemap", ":nomeTema") }}'.replace(':nomeTema', '{{ $temaAtivo }}');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (llmsForm) {
        llmsForm.addEventListener('submit', function(e) {
            ...
            const url = '{{ route("dashboard.temas.generate-llms", ":nomeTema") }}'.replace(':nomeTema', currentTemaForLlms);
...
document.getElementById('newPageName').addEventListener('input', function() {
    ...
    if (newName) {
        ...
        routePreview.textContent = '{{ url("/") }}' + '/' + routeName;
